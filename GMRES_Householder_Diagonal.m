function [W, H, beta] = GMRES_Householder_Diagonal(a_val, a_row_ptr, a_col_idx, r0, m, m_diagonal_val)
...
% W: the Householder unit vectors; H, beta: for GMRES
	...
	for j = 1 : m + 1
		...
		v = dotproduct(a_val, a_row_ptr, a_col_idx, v);
		v = v ./ m_diagonal_val;    % Left Preconditioning
		for k = 1 : j
		    v = v - W(:, k) * (2 * (W(:, k)' * v));
		end
		z = v;
